...
from safe_mpc.utils import get_controller, randomize_model, get_ocp_acados
...
# OCP to restart after backup
params_restarter = Parameters(args,model_name, rti=False)
params_restarter.q_margin = args['joint_bounds_margin']
params_restarter.collision_margin = args['collision_margin']

params_restarter.build = args['build']
params_restarter.act = args['activation']
params_restarter.alpha = args['alpha']
params_restarter.backhor = args['back_hor']
params_restarter.N=horizon
model_restarter = AdamModel(params_restarter)


model_restarter.ee_ref = params_restarter.ee_ref
nq = model_restarter.nq

params_restarter.noise_mass = args['noise']
params_restarter.noise_inertia = args['noise']
params_restarter.noise_cm = args['noise']
params_restarter.control_noise = args['control_noise']
build_controllers=args['build']
...
controller_restarter,_ = get_ocp_acados(cont_name, model_restarter)
cost_controller_restarter = ReachTargetNLS(model_restarter,params_restarter.Q_weight,params_restarter.R_weight)
cost_controller_restarter.set_solver_cost(controller_restarter)
controller_restarter.build_controller(build_controllers)
    ...
    for j in range(params.n_steps):
        ...
        if sa_flag and ja < safe_ocp.N:
            ...
            if ja == safe_ocp.N:
                ...
                xg_restart = np.full((controller_restarter.N + 1, model.nx), x_sim[j])
                ug_restart = np.zeros((controller_restarter.N, model.nu))
                controller_restarter.setGuess(xg_restart,ug_restart)
                status = controller_restarter.solve(x_sim[j])
                if (((status == 0) or (status==2)) and controller_restarter.checkGuess()):
                    print("Restart successful")
                    controller.x_guess = controller_restarter.x_temp
                    controller.u_guess = controller_restarter.u_temp
                else:
                    print('Restart failed: try simple RTI')
        else:   
